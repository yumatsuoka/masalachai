...
#
labeled_data = datafeeders.SiameseFeeder(labeled_data_dict, batchsize=args.lbatch)
...
# Model Setup
h_units = 1200
model = models.SiameseModel(Mlp(labeled_data['data'][0].size, h_units, h_units, np.max(labeled_data['target'])+1))
if args.gpu >= 0:
