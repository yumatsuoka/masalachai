...
class DataFeeder(object):
    ...
    def get_data_dict_list(self, indexes):
        return [{k : v[i] if getattr(v,'__iter__',False) and len(v)==self.n else v for k,v in self.data_dict.items()} for i in indexes]
        ...
        while not self.stop.is_set():
            ...
            while cnt < self.n and not self.stop.is_set():
                ...
                for i in range(len(indexes)):
                    batch_pool.append(pool.apply_async(preprocess, (self.preprocess_hooks, data_dict_list[i])))
