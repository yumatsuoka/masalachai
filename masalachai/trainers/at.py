...
class AdversarialTrainer(SupervisedTrainer):
    def __init__(self, optimizer, logger, train_data_feeders, test_data_feeder, gpu=-1,
                 eps=4.0, alpha=0.5):
        super(AdversarialTrainer, self).__init__(optimizer, logger, train_data_feeders, test_data_feeder=test_data_feeder, gpu=gpu)
        self.eps = eps
        self.alpha = alpha
    ...
    def adversarial_loss(self, vx, vt):
        ...
        adv_vx = tuple( [ chainer.Variable(self.eps * xp.sign(x.grad) + x.data) for x in vx] )
