...
def siamese_preprocess(inp):
    ...
    data['target'] = numpy.asarray(1).astype(numpy.int32)\
        if data0['target'] == data1['target']\
        else numpy.asarray(0).astype(numpy.int32)
    return data
                    ...
                    batch_pool.append(
                        pool.apply_async(
                            ...
                            (self.preprocess_hooks,
                             (data_dict_list0[i], data_dict_list1[i]))
                        )
                    )
